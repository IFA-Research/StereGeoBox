...
for left_name in tqdm(left_images, desc="RAFT"):
    ...
    npz_path = os.path.join(args.depth_dir, f"{base}.npz")
    npy_path = os.path.join(args.depth_dir, f"{base}.npy")
    if os.path.exists(npz_path):
        disp = np.load(npz_path)["disp"].astype(np.float32)
    elif os.path.exists(npy_path):
        disp = np.load(npy_path).astype(np.float32)
    else:
        continue
    depth_map = (args.fx * args.baseline) / (np.abs(disp) + 1e-6)
...
if all_results:
    pd.DataFrame(all_results).to_excel(os.path.join(args.output_dir, "results_RAFT.xlsx"), index=False)
